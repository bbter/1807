...
string:     目标字符串
count:      最多替换次数
功能:在目标字符串中以正则表达式的规则匹配字符串,在把他们替换成指定字符串
，可以指定替换的次数，如果不指定,替换所有的匹配字符串

区别:前者返回一个被替换的字符串,后者返回一个元组，第一个元素被替换的字符串
,第二个元素表示被替换的次数
'''
...
print(re.match(pat,"13868864676"))
# 编译成正则对象
re_telephone = re.compile(pat)
print(re_telephone.match("13868864676"))
...
print(re_QQ.search("1235656515567890"))